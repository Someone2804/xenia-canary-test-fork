...
// Optional instrumentation for Fable II morph debugging.
#define XE_FABLE2_TRACE_MORPH 0
#define XE_FABLE2_SOLID_COLOR 0
...
D3D12TextureCache::D3D12Texture::D3D12Texture(
  ...
  if (shared_resource_state_) {
    SetHostMemoryUsage(0);
  } else {
    SetHostMemoryUsage(
...
void D3D12TextureCache::AdjustHostRepackForKey(TextureKey& key) {
...
std::unique_ptr<TextureCache::Texture> D3D12TextureCache::CreateTexture(
  ...
  if (IsMorphTextureFormat(key.format)) {
    ...
    if (!command_processor_.render_target_cache().EnsureHostSurfaceForTexture(
            surface_key, resource, shared_state, seeded, &rtv_format) ||
        resource == nullptr || shared_state == nullptr) {
      return nullptr;
    }

#if XE_FABLE2_TRACE_MORPH
    {
      const D3D12_RESOURCE_DESC surface_desc = resource->GetDesc();
      uint64_t log_hash = (uint64_t(surface_key.base_page) << 32) ^
                          (uint64_t(surface_key.width) << 16) ^
                          uint64_t(surface_key.height) ^
                          (uint64_t(surface_key.texture_format) << 48) ^
                          (uint64_t(surface_key.signed_mask) << 8);
      static std::unordered_set<uint64_t> logged_morph_surfaces;
      if (logged_morph_surfaces.insert(log_hash).second) {
        XELOGI(
            "[Fable2Morph] HostSurface create base=%08X size=%ux%u fmt=%u "
            "signed=%u tiled=%u seeded=%u host_fmt=%u rtv_fmt=%u",
            surface_key.base_page, surface_key.width, surface_key.height,
            static_cast<uint32_t>(surface_key.texture_format),
            static_cast<uint32_t>(surface_key.signed_mask), key.tiled ? 1 : 0,
            seeded ? 1 : 0, static_cast<uint32_t>(surface_desc.Format),
            static_cast<uint32_t>(rtv_format));
      }
    }
#endif  // XE_FABLE2_TRACE_MORPH

    key.host_repacked = 1;
    D3D12_RESOURCE_STATES current_state = *shared_state;
    return std::unique_ptr<TextureCache::Texture>(
        new D3D12Texture(*this, key, resource, current_state, shared_state));
  }
...
bool D3D12TextureCache::LoadTextureDataFromResidentMemoryImpl(Texture& texture,
  ...
  if (IsMorphTextureFormat(texture_key.format)) {
    ...
    surface_key.width = texture_key.GetWidth();
    surface_key.height = texture_key.GetHeight();
    surface_key.signed_mask = static_cast<uint8_t>(texture_key.signed_mask);
    ID3D12Resource* host_surface_resource = nullptr;
    D3D12_RESOURCE_STATES* host_surface_state = nullptr;
    bool host_surface_seeded = false;
    command_processor_.render_target_cache().EnsureHostSurfaceForTexture(
        surface_key, host_surface_resource, host_surface_state,
        host_surface_seeded);
    if (host_surface_seeded) {
      return true;
    }
  ...
  D3D12RenderTargetCache::HostSurfaceTextureKey host_surface_key;
  if (adopt_host_surface) {
    host_surface_key.base_page = texture_key.base_page;
    host_surface_key.texture_format = texture_key.format;
    host_surface_key.width = texture_key.GetWidth();
    host_surface_key.height = texture_key.GetHeight();
    host_surface_key.signed_mask = static_cast<uint8_t>(texture_key.signed_mask);
  }

  if (adopt_host_surface) {
    ID3D12Resource* host_surface_resource_unused = nullptr;
    D3D12_RESOURCE_STATES* host_surface_state_unused = nullptr;
    bool host_surface_seeded_unused = false;
    command_processor_.render_target_cache().EnsureHostSurfaceForTexture(
...
uint32_t D3D12TextureCache::FindOrCreateTextureDescriptor(
  ...
  bool host_declares_signed = texture_key.signed_mask != 0;
  bool use_signed_view = is_signed && host_declares_signed;

  D3D12Texture::SRVDescriptorKey descriptor_key;
  descriptor_key.is_signed = uint32_t(use_signed_view);
  ...
  if (IsMorphTextureFormat(texture_key.format)) {
    if (IsK8888A(texture_key.format)) {
      ...
      DXGI_FORMAT resource_format = texture.resource()->GetDesc().Format;
      switch (resource_format) {
        case DXGI_FORMAT_R16_TYPELESS:
          desc.Format = use_signed_view ? DXGI_FORMAT_R16_SNORM
                                        : DXGI_FORMAT_R16_UNORM;
          break;
        case DXGI_FORMAT_R16G16_TYPELESS:
          desc.Format = use_signed_view ? DXGI_FORMAT_R16G16_SNORM
                                        : DXGI_FORMAT_R16G16_UNORM;
          break;
        case DXGI_FORMAT_R16G16B16A16_TYPELESS:
          desc.Format = use_signed_view ? DXGI_FORMAT_R16G16B16A16_SNORM
  ...
  if (IsMorphTextureFormat(texture_key.format)) {
    uint64_t log_hash = (uint64_t(texture_key.base_page) << 32) ^
                        ...
                        (use_signed_view ? 0x1 : 0x0);
    static std::unordered_set<uint64_t> logged_morph_srvs;
    if (logged_morph_srvs.insert(log_hash).second) {
      XELOGI(
          "[Fable2Morph] SRV create base=%08X fmt=%u signed=%u dxgi=%u mipMax=%u",
          texture_key.base_page, static_cast<uint32_t>(texture_key.format),
          use_signed_view ? 1u : 0u, static_cast<uint32_t>(desc.Format),
          texture_key.mip_max_level);
    }
  }
#endif  // XE_FABLE2_TRACE_MORPH
...

  {
    if (k.format == xenos::TextureFormat::k_8_8_8_8 ||
      ...
      // Избежать SNORM: всегда UNORM
      desc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
      // k_8_8_8_8_A — только mip0
      if (k.format == xenos::TextureFormat::k_8_8_8_8_A) {
...
bool D3D12TextureCache::UploadPlain16BitTexture(D3D12Texture& texture,
  ...
  const TextureKey& key = texture.key();
  uint32_t component_count = GetComponentCountForPlain16Bit(key.format);
  if (!component_count) {
    return false;
  }
  ...
  const texture_util::TextureGuestLayout::Level& base_layout = layout.base;
  const FormatInfo* format_info = FormatInfo::Get(key.format);
  uint32_t bpb = format_info->bytes_per_block();
  if (!bpb) {
    return false;
  }

  uint32_t row_pitch_bytes = base_layout.row_pitch_bytes;
  if (!row_pitch_bytes) {
    return false;
  }
  uint32_t pitch_blocks = row_pitch_bytes / bpb;
  uint32_t bpb_log2 = xe::log2_floor(bpb);
  uint32_t bytes_per_texel = component_count * sizeof(uint16_t);

  uint32_t base_address = key.base_page << 12;
  uint32_t base_size = base_layout.array_slice_data_extent_bytes;
  if (!base_size) {
    base_size = row_pitch_bytes * height;
  }
...
#if !XE_FABLE2_SOLID_COLOR
  if (!shared_memory().RequestRange(
          base_address, xe::align(base_size, uint32_t(16)), nullptr)) {
    return false;
  }
  const auto& d3d12_shared_memory =
      static_cast<const D3D12SharedMemory&>(shared_memory());
  const uint8_t* base_ptr = d3d12_shared_memory.TranslatePhysical(base_address);
  if (!base_ptr) {
    return false;
  }
#endif
  ...
  D3D12_RESOURCE_DESC texture_desc = texture.resource()->GetDesc();
  DXGI_FORMAT upload_format = DXGI_FORMAT_R16G16B16A16_FLOAT;
  uint32_t host_components = 4;
  switch (texture_desc.Format) {
    case DXGI_FORMAT_R16_TYPELESS:
      upload_format = DXGI_FORMAT_R16_FLOAT;
      host_components = 1;
      break;
    case DXGI_FORMAT_R16G16_TYPELESS:
      upload_format = DXGI_FORMAT_R16G16_FLOAT;
      host_components = 2;
      break;
    case DXGI_FORMAT_R16G16B16A16_TYPELESS:
      upload_format = DXGI_FORMAT_R16G16B16A16_FLOAT;
      host_components = 4;
      break;
    default:
      upload_format = DXGI_FORMAT_R16G16B16A16_FLOAT;
  ...
  std::vector<uint16_t> staging(width * height * host_components, half_zero);

#if XE_FABLE2_SOLID_COLOR
  for (uint32_t i = 0; i < width * height; ++i) {
    size_t idx = size_t(i) * host_components;
    staging[idx + 0] = half_one;
    if (host_components > 3) {
      staging[idx + 3] = half_one;
    }
  }
#else
  for (uint32_t y = 0; y < height; ++y) {
    for (uint32_t x = 0; x < width; ++x) {
      ...
      if (!key.tiled) {
        texel_ptr = base_ptr + uint64_t(y) * row_pitch_bytes +
                    uint64_t(x) * bytes_per_texel;
      } else {
        uint32_t offset = uint32_t(texture_util::GetTiledOffset2D(
            int32_t(x), int32_t(y), pitch_blocks, bpb_log2));
        texel_ptr = base_ptr + offset;
      }
      size_t idx = (size_t(y) * width + x) * host_components;
      if (host_components >= 4) {
        staging[idx + 3] = half_one;
      }
      for (uint32_t c = 0; c < component_count && c < host_components; ++c) {
        uint16_t raw;
        std::memcpy(&raw, texel_ptr + c * sizeof(uint16_t), sizeof(raw));
        raw = xenos::GpuSwap(raw, key.endianness);
        float v;
        if (((key.signed_mask >> c) & 0x1) != 0) {
          v = std::clamp(float(int16_t(raw)) / 32767.0f, -1.0f, 1.0f);
        } else {
...

  {
    ...
    uint8_t* dst = map + footprint.Offset;
    const size_t src_row_bytes = size_t(width) * host_components * sizeof(uint16_t);
    for (UINT row = 0; row < num_rows; ++row) {
      uint8_t* dst_row = dst + row * footprint.Footprint.RowPitch;
      ...
      size_t padding = footprint.Footprint.RowPitch > src_row_bytes
                           ? size_t(footprint.Footprint.RowPitch) -
                                 src_row_bytes
                           : 0u;
      if (padding) {
        std::memset(dst_row + src_row_bytes, 0, padding);
...
void D3D12TextureCache::InvalidateHostSurfaceBindings(
    uint32_t base_page, xenos::TextureFormat format, uint32_t width,
    uint32_t height, uint8_t signed_mask) {
  uint32_t cleared_bindings = 0;
  const uint32_t binding_count = uint32_t(texture_bindings_.size());
  for (uint32_t i = 0; i < binding_count; ++i) {
    TextureBinding& binding = texture_bindings_[i];
    if (!binding.key.is_valid || !binding.key.host_repacked) {
      continue;
    }
    if (binding.key.base_page != base_page ||
        binding.key.format != format ||
        binding.key.GetWidth() != width ||
        binding.key.GetHeight() != height ||
        binding.key.signed_mask != signed_mask) {
      continue;
    }

    const xenos::DataDimension dimension = binding.key.dimension;
    D3D12TextureBinding& d3d12_binding = d3d12_texture_bindings_[i];

    uint32_t descriptor_index_unsigned = d3d12_binding.descriptor_index;
    uint32_t descriptor_index_signed = d3d12_binding.descriptor_index_signed;
    if (descriptor_index_unsigned != UINT32_MAX) {
      WriteNullSRVDescriptor(descriptor_index_unsigned, dimension);
    }
    if (descriptor_index_signed != UINT32_MAX &&
        descriptor_index_signed != descriptor_index_unsigned) {
      WriteNullSRVDescriptor(descriptor_index_signed, dimension);
    }

    binding.Reset();
    d3d12_binding.Reset();
    cleared_bindings |= UINT32_C(1) << i;
  }
  if (cleared_bindings != 0) {
    texture_bindings_in_sync_ &= ~cleared_bindings;
  }
}

void D3D12TextureCache::WriteNullSRVDescriptor(
    uint32_t descriptor_index, xenos::DataDimension dimension) {
  if (descriptor_index == UINT32_MAX) {
    return;
  }
  NullSRVDescriptorIndex null_descriptor_index =
      NullSRVDescriptorIndex::k2DArray;
  switch (dimension) {
    case xenos::DataDimension::k3D:
      null_descriptor_index = NullSRVDescriptorIndex::k3D;
      break;
  ...
  const ui::d3d12::D3D12Provider& provider =
      command_processor_.GetD3D12Provider();
  D3D12_CPU_DESCRIPTOR_HANDLE source_handle = provider.OffsetViewDescriptor(
      null_srv_descriptor_heap_start_, uint32_t(null_descriptor_index));
  D3D12_CPU_DESCRIPTOR_HANDLE dest_handle =
      GetTextureDescriptorCPUHandle(descriptor_index);
  provider.GetDevice()->CopyDescriptorsSimple(
      1, dest_handle, source_handle, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
}
